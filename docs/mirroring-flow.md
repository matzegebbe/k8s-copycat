...
flowchart TD
    ...
    N -- Yes --> Z[End]
    N -- No --> R{Allowed to overwrite different digest?<br/>(`latest` tag or allowDifferentDigestRepush)}
...
flowchart TD
    ...
    J -- No --> L{Allowed to overwrite different digest?<br/>(`latest` tag or allowDifferentDigestRepush)}
